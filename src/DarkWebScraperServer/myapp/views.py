...
def driving_score(request):
    ...
    driver_score_simulation.input['peak'] =  request.GET.get('peak',5)
    driver_score_simulation.input['road_condition'] = request.GET.get('roadCondition',1)
    if driver_score_simulation.input['road_condition'] == 1:
        speed_val = 25
    else:
        speed_val = 45
    driver_score_simulation.input['speed'] = request.GET.get('speed', speed_val)

    driver_score_simulation.compute()
    drvingScoreVal = driver_score_simulation.output['driver_score']
    response = HttpResponse(drvingScoreVal)
    response['Access-Control-Allow-Origin'] = 'null'
    return response
